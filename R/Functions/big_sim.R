main_function <- function(longitude, latitude, 
  ...
  function_2 <- function(micro, ecto) {
    ...
    pars <- readMat(paste0('R/DEB/', species, '/results_', species, '.mat'))
    par.names <- unlist(labels(pars$par))
    for(i in 1:length(par.names)){
      assign(par.names[i], unlist(pars$par[i]))
    }
    E.0 <- 1145 # energy to create one egg
    L.b <- 0.4213 # length at birth
  ...
  # Do something with results_2 or return it
  return(results_2)
}
...
main_function(longitude = 151.004, #145.003
              latitude =  -33.8,#-37.8, 
...
#################
# Function and loop attempt with the same code. 
run_sim_all <- function(longitude, latitude, 
                       species, # delicata or guchi
                       region, # AUS, HA, NZ
                       spatial) # what spatial folder to grab ERA5 data 
  {
  
  #############  #############  #############
  ############# Microclimate model
  #############  #############  #############
  # parameter list for microclimate  & ectotherm model for each species
  source(paste0('R/life cycle models/', species, '/parameters_biophys.R'))
  loc <- c(longitude, latitude) 
  nyears <- yfinish - ystart + 4
...
# Example dataset  
run_sim_all(longitude = 151.004, #145.003
            latitude =  -33.8,#-37.8, 
            species = "Lampropholis_delicata",
            spatial = "Data/ERA_5/Australia/ERA5", 
            region = "AUS") 
...
runMicroclimateModel <- function(longitude, latitude) {
  source(paste0('R/life cycle models/', species, '/parameters_biophys.R'))
  # Run microclimate model
  micro <- micro_era5(windfac = windfac, 
                      RUF = RUF, 
